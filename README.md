...
### 🚀 방법 A — 무설치 실행 (권장, 일반 사용자)
1. GitHub **[Releases](https://github.com/black-astro/coding-test/releases)** 에서 최신 `codeT-windows.zip` 다운로드
2. 압축 풀기 → 폴더 안의 **`codeT.exe`** 더블클릭
3. 끝! Python 은 내장되어 있어 따로 설치할 필요 없습니다.
   - Java/C++ 로도 풀려면 JDK / g++ 가 함께 동봉된 패키지를 받거나, 직접 설치(앱 안 **가이드** 참고)
...
| 섹션 | 내용 |
|------|------|
| **언어** | 문법 강의 — **Python·Java·C++·JavaScript × 기초·중급·고급** + **CSS·SCSS** (설명+예시+사용처+단점, 직접 실행/컴파일) |
| **영단어** | 개발자 IT 용어·은어 + 일반 영어 약 2,400+개 (기초/중급/고급) — 예문과 함께 학습, **퀴즈로 테스트**(점수 sqlite 저장) |
| **랭크** | 난이도별 문제 — 🥉 Bronze / 🥈 Silver / 🥇 Gold / 💎 Platinum (실전·종목 문제도 난이도에 맞게 포함) |
| **실전** | 유형별 — 구현/DFS·BFS/DP/그리디/해시/이분탐색/세그트리 등 대기업 빈출 유형 |
| **종목** | 알고리즘 종목별 — 같은 유형을 **기초→고급** 난이도순으로 |
| **시험** | **🏁 실전 코딩테스트 도전** — 누르면 한국 대기업 실전형으로 **매번 랜덤 출제**, 제한시간, 합격/불합격 판정 |
| **가이드** | 환경 설정 — JDK/g++/Node 설치·적용·사용법 |

> 같은 문제가 **랭크(난이도별)** 와 **종목(유형별)** 양쪽에 나옵니다 — 보는 관점만 다릅니다.
> 헤더 좌측엔 **현재 티어 + 게이지**가 표시되고, 현재 티어보다 쉬운 문제를 풀면 게이지는 오르지 않습니다.
> 헤더 **⚙ 설정**에서 *문제 변형 리셋 / 내 티어 초기화 / 환경 점검* 을 할 수 있습니다.
...
**성능 조건(시간·메모리)은 문제마다 개별 적용**됩니다 — 각 문제가 자신의 `time_limit_ms`/`memory_limit_mb`
를 가지며(미지정 시 랭크 기본값), 채점기가 케이스별 실행 시간·메모리를 측정해 초과하면 TLE/MLE 처리합니다.
