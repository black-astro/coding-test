...
LESSONS = [
    ...
    Lesson(
        ...
        summary="if / else if / else · 중첩 if · switch · 삼항 연산자",
        explanation=(
            "[개요]\n"
            "조건문은 조건의 참/거짓에 따라 실행할 코드를 선택하는 제어 구조이다. 프로그램이 \"만약 ~라면 이것을, 아니라면 저것을\" 수행하도록 흐름을 분기하며, 거의 모든 프로그램의 판단 로직이 조건문으로 작성된다.\n"
            "\n"
            "[핵심 개념]\n"
            "if / else if / else:\n"
            "• if (조건)        — 조건이 참(true)이면 중괄호 { } 안의 코드를 실행한다.\n"
            "• else if (조건2)  — 앞선 조건이 거짓이고 조건2가 참이면 실행한다. 여러 개를 이어 붙일 수 있다.\n"
            "• else             — 위의 모든 조건이 거짓일 때 실행한다.\n"
            "• 위에서부터 차례로 검사하며, 처음으로 참이 된 블록 하나만 실행된다.\n"
            "\n"
            "조건 만들기(비교·논리 연산자의 활용):\n"
            "• 비교 연산자 ==, !=, <, <=, >, >= 의 결과는 true 또는 false이며, 이것이 if의 조건이 된다.\n"
            "• 논리 연산자로 조건을 조합한다: && (그리고 — 둘 다 참), || (또는 — 하나라도 참), ! (부정)\n"
            "• 예: age >= 19 && hasTicket — 나이가 19 이상이면서 표도 있어야 참이다.\n"
            "\n"
            "중첩 if:\n"
            "• if 블록 안에 다시 if를 넣을 수 있다. 조건을 단계적으로 좁혀갈 때 사용한다.\n"
            "\n"
            "switch:\n"
            "• 하나의 값을 여러 경우(case)와 비교하여 분기한다. 각 case 끝에는 break를 두어야 다음 case로 이어지지 않는다.\n"
            "• 어떤 case에도 해당하지 않으면 default 블록이 실행된다.\n"
            "\n"
            "삼항 연산자:\n"
            "• 형식: 조건 ? 참일때값 : 거짓일때값 — 간단한 if/else를 한 줄로 표현한다.\n"
            "\n"
            "[코드 분석]\n"
            "• int score = 85;                    → 점수 변수 score를 85로 초기화한다.\n"
            "• if (score >= 90) {                 → score가 90 이상인지 검사한다. 85이므로 거짓이라 이 블록은 건너뛴다.\n"
            "• } else if (score >= 80) {          → 앞 조건이 거짓이고 score가 80 이상이므로 참이다. \"등급: B\"가 출력된다.\n"
            "• } else {                           → 위 조건이 모두 거짓일 때만 실행된다. 여기서는 실행되지 않는다.\n"
            "• if (age >= 19 && hasTicket) {      → 나이가 19 이상이고(&&) 표가 있어야 참이다. 두 조건 모두 참이므로 \"입장 가능\"이 출력된다.\n"
            "• if (n % 2 == 0) {                  → n을 2로 나눈 나머지가 0이면(짝수) 안으로 들어간다. %는 나머지 연산자이다.\n"
            "• if (n % 3 == 0) {                  → 중첩 if. 짝수이면서 3의 배수이면 6의 배수이다. 12는 두 조건을 모두 만족한다.\n"
            "• switch (menu) {                    → menu의 값과 일치하는 case로 이동한다.\n"
            "• case 2: ... break;                 → menu가 2이므로 \"메뉴: 주스\"를 출력하고 break로 switch를 빠져나간다.\n"
            "• default: ... break;                → 일치하는 case가 없을 때 실행된다. 여기서는 실행되지 않는다.\n"
            "• int mx = (a > b) ? a : b;          → 삼항 연산자. a(7)가 b(3)보다 크므로 mx에 a가 대입된다.\n"
            "\n"
            "[유의 사항]\n"
            "• 비교는 ==, 대입은 = 이다. if (a = 5)는 비교가 아닌 대입이 되어 의도치 않은 버그가 발생한다.\n"
            "• else if 는 위에서부터 순서대로 검사하므로, 점수 등급처럼 범위가 겹치는 조건은 큰 값부터(>= 90 → >= 80) 배치해야 의도대로 동작한다.\n"
            "• switch의 case 끝에 break를 빠뜨리면 다음 case까지 연달아 실행된다(fall-through).\n"
            "• switch는 정수·char 값에만 사용할 수 있다. string 비교에는 if/else를 사용한다.\n"
            "• 삼항 연산자는 간단한 선택에만 쓰고, 복잡한 분기는 if/else로 풀어 쓰는 것이 읽기 쉽다."
        ),
        usage="조건에 따라 동작을 바꾸는 모든 문제의 기본. 시뮬레이션의 규칙 판정, 점수/범위 분기, 짝홀 판별 등 코딩 테스트 전 영역에 등장한다.",
        cons="== 와 = 혼동, switch의 break 누락 같은 실수가 잦다. 분기가 많아지면 조건의 배치 순서에 따라 결과가 달라지므로 주의해야 한다.",
        code=(
            "#include <bits/stdc++.h>\n"
            "using namespace std;\n"
            "\n"
            "int main() {\n"
            "    // if / else if / else — 점수에 따라 등급 판정\n"
            "    int score = 85;\n"
            "    if (score >= 90) {\n"
            "        cout << \"등급: A\" << '\\n';\n"
            "    } else if (score >= 80) {\n"
    ...
    Lesson(
        ...
        lang="cpp", level="기초",
        title="반복문 (for · while)",
        summary="for · while · do-while · break/continue · 범위 기반 for",
        explanation=(
            "[개요]\n"
            "반복문은 동일한 코드 블록을 여러 번 반복 실행하는 제어 구조이다. 100개의 입력을 처리하거나 1부터 N까지의 합을 구하는 등, 같은 작업의 반복을 코드 몇 줄로 표현한다.\n"
            "\n"
            "[핵심 개념]\n"
            "for — 반복 횟수가 정해져 있을 때 사용한다:\n"
            "• 형식: for (시작; 계속조건; 변화) { 반복할 내용 }\n"
            "• 예: for (int i = 1; i <= 5; i++) — i를 1로 시작하여 5 이하인 동안 매 반복마다 1씩 증가시킨다(총 5회).\n"
            "\n"
            "while — 반복 횟수를 미리 알 수 없고 조건으로 제어할 때 사용한다:\n"
            "• 형식: while (조건) { 반복할 내용 }\n"
            "• 조건이 참인 동안 반복하며, 조건이 거짓이 되면 종료한다.\n"
            "\n"
            "do-while — 본문을 먼저 1회 실행한 뒤 조건을 검사한다:\n"
            "• 형식: do { 반복할 내용 } while (조건);\n"
            "• 조건이 처음부터 거짓이어도 본문이 최소 1회는 실행된다는 점이 while과 다르다.\n"
            "\n"
            "break / continue — 반복의 흐름을 제어한다:\n"
            "• break    — 반복을 즉시 완전히 종료한다.\n"
            "• continue — 현재 회차의 남은 코드를 건너뛰고 다음 회차로 진행한다.\n"
            "\n"
            "범위 기반 for(맛보기):\n"
            "• 형식: for (int x : v) — vector 등 모음의 원소를 처음부터 끝까지 하나씩 x에 담아 순회한다. 인덱스가 필요 없을 때 간결하다.\n"
            "\n"
            "[코드 분석]\n"
            "• for (int i = 1; i <= 5; i++) {     → i를 1부터 5까지 1씩 증가시키며 총 5회 반복한다.\n"
            "• cout << i << ' ';                  → 현재 i 값과 공백을 출력한다. 결과는 1 2 3 4 5.\n"
            "• int n = 5;                         → while에서 사용할 변수 n을 5로 초기화한다.\n"
            "• while (n > 0) {                    → n이 0보다 큰 동안 반복한다.\n"
            "• n--;                               → n을 1 감소시킨다. 이 줄이 없으면 조건이 계속 참이라 무한 루프가 된다.\n"
            "• int k = 10;                        → do-while에서 사용할 변수 k를 10으로 초기화한다.\n"
            "• do {                               → do-while은 조건 검사보다 본문 실행이 먼저다.\n"
            "• } while (k < 5);                   → k가 10이라 조건은 거짓이지만, 본문은 이미 1회 실행된 뒤이다.\n"
            "• if (i % 2 == 1) continue;          → i가 홀수이면 아래 코드를 건너뛰고 다음 회차로 넘어간다.\n"
            "• if (i > 10) break;                 → i가 10을 넘는 순간 반복을 즉시 종료한다.\n"
            "• total += i;                        → 2~10의 짝수만 누적된다. 2+4+6+8+10 = 30.\n"
            "• vector<int> v = {3, 1, 2};         → 정수 3개를 담은 vector를 선언한다.\n"
            "• for (int x : v) cout << x << ' ';  → 범위 기반 for. v의 원소를 앞에서부터 하나씩 x에 담아 출력한다.\n"
            "\n"
            "[유의 사항]\n"
            "• while 내부에서 조건 변수를 변경하지 않으면 무한 루프가 발생한다. 반드시 탈출 조건을 마련한다.\n"
            "• 배열/vector를 순회하는 for의 인덱스는 0부터 시작한다(0 <= i < 크기). i <= 크기로 쓰면 범위 초과 오류이다.\n"
            "• break는 가장 안쪽 반복 하나만 종료한다. 이중 반복 전체를 끝내려면 플래그 변수나 함수 return을 활용한다.\n"
            "• while에서 continue를 쓸 때 증감 코드가 continue 뒤에 있으면 건너뛰어져 무한 루프가 될 수 있다.\n"
            "• 경계값(1 <= i vs 1 < i, i <= n vs i < n)을 잘못 잡으면 한 번 덜 돌거나 더 도는 off-by-one 오류가 난다."
        ),
        usage="N개의 입력 처리, 합/개수 세기, 2차원 배열 순회 등 모든 코딩 테스트 문제의 뼈대다. 시뮬레이션 유형은 사실상 반복문과 조건문의 조합이다.",
        cons="조건이나 증감을 잘못 쓰면 무한 루프 또는 off-by-one 오류가 난다. 이중 반복에서 i와 j를 혼동하는 실수도 잦다.",
        code=(
            "#include <bits/stdc++.h>\n"
            "using namespace std;\n"
            "\n"
            "int main() {\n"
            "    // for — 1부터 5까지 출력 (횟수가 정해진 반복)\n"
            "    for (int i = 1; i <= 5; i++) {\n"
            "        cout << i << ' ';\n"
            "    }\n"
            "    cout << '\\n';   // 1 2 3 4 5\n"
            "\n"
            "    // while — 조건이 참인 동안 반복 (5부터 거꾸로)\n"
            "    int n = 5;\n"
            "    while (n > 0) {\n"
            "        cout << n << ' ';\n"
            "        n--;\n"
            ...
            "    } while (k < 5);\n"
            "\n"
            "    // break / continue — 2~10 의 짝수만 누적\n"
            "    int total = 0;\n"
            "    for (int i = 1; i <= 100; i++) {\n"
            "        if (i % 2 == 1) continue;   // 홀수는 건너뛴다\n"
            "        if (i > 10) break;          // 10을 넘으면 반복 종료\n"
            "        total += i;\n"
            "    }\n"
            "    cout << \"짝수합(2~10): \" << total << '\\n';   // 30\n"
            "\n"
            "    // 범위 기반 for 맛보기 — vector 의 원소를 차례로 순회\n"
            "    vector<int> v = {3, 1, 2};\n"
            "    for (int x : v) cout << x << ' ';\n"
            "    cout << '\\n';   // 3 1 2\n"
            "    return 0;\n"
